// LUNE — interactions, animations, mobile responsive
(function () {
  'use strict';
  ...
  function buildSearch() {
    ...
    sov.innerHTML = `
      <div style="display:flex; justify-content:space-between; align-items:center; max-width:720px; margin:0 auto;">
        <span style="font-family:'Cormorant Garamond',serif; font-size:18px; letter-spacing:.3em;">LUNE</span>
  ...
  function buildNewsletter() {
    if (sessionStorage.getItem('lune_news_dismissed')) return;
    ...
    const close = () => {
      ...
      document.body.style.overflow = '';
      sessionStorage.setItem('lune_news_dismissed', '1');
    };
    ...
    let triggered = false;
    const trigger = () => {
      if (triggered || sessionStorage.getItem('lune_news_dismissed')) return;
      triggered = true;
  ...
  function applyCategory() {
    ...
    if (searchTerm) {
      ...
      document.title = `Search: ${searchTerm} — LUNE`;
      return;
    }
    ...
    if (heading && labels[cat]) {
      ...
      document.title = labels[cat] + ' — LUNE';
    }
